...
#if 1
  // Change the above line to "#if 1", and your code goes here
  yfs->writefile(ino,buf,off,size);
                        ...
                        mode_t mode, struct fuse_entry_param *e)
{
  ...
  /*if(yfs->isdir(parent_ino) == false)
  {
    return yfs_client::NOENT;
  }*/
  
   yfs_client::inum ino = rand()%MAX + MAX;
   int exist = yfs->createfile(parent_ino,ino,name);
   //cout<<"fuse.cc"<<exist<<endl;
   if(exist == -2)
   {
...
fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  e.generation = 0;
  bool found = false;
  yfs_client::inum fileid=-1;
  yfs_client::inum pa = parent;
  yfs->fileindir2(pa,name,fileid);
                   ...
                   off_t off, struct fuse_file_info *fi)
{
  ...
  memset(&b, 0, sizeof(b));
  map<yfs_client::inum,string> itemvec;
  if(yfs->getallitembydir(inum,itemvec) == yfs_client::OK)
  {
  	for(map<yfs_client::inum,string>::iterator i=itemvec.begin();i!= itemvec.end();i++)
	{
		fuse_ino_t itemino = i->first;
		dirbuf_add(&b,(i->second).c_str(),itemino);
	}
  }
...
#if 1
	yfs_client::inum parentino = parent;
	yfs_client::inum dirnum = rand()%MAX;
	int r = yfs->createdir(parentino,dirnum,name);
	if(r == 1)
	{
		if(getattr(dirnum,e.attr) == yfs_client::OK)
		{
			e.ino = dirnum;
			fuse_reply_entry(req, &e);
		}
...
fuseserver_unlink(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  yfs_client::inum parentino = parent;
  int r = yfs->unlinkfile(parentino,name);
  if(r == 0)
  {
	fuse_reply_err(req, ENOENT);
  }
