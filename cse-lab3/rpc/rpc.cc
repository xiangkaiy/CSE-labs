...
{
	ScopedLock rwl(&reply_window_m_);
	if(real_xid_rep.count(clt_nonce) == 0)
	{
		real_xid_rep[clt_nonce] = xid_rep;
	}
	else
	{
		if(xid_rep > real_xid_rep[clt_nonce])
		{
			real_xid_rep[clt_nonce] = xid_rep;
		}
	}
	list<reply_t> reply_list = reply_window_[clt_nonce];
	bool exist = false;
	if(reply_list.size() != 0)
	{
		for(list<reply_t>::iterator i=reply_list.begin();i!=reply_list.end();i++)
		{
			if(i->xid == xid)
			{
				exist = true;
				if(i->cb_present == true)
				{
					*b = i->buf;
					*sz = i->sz;
					return DONE;
		...
		if(exist == false)
		{
			...
			{
				reply_t h(xid);
				reply_window_[clt_nonce].push_back(h);
				return NEW;
			}
		}
	}
	...
	{
		reply_t y(xid);
		reply_window_[clt_nonce].push_back(y);
		return NEW;
	}
}
		...
		char *b, int sz)
{
	...
	for(list<reply_t>::iterator i=reply_window_[clt_nonce].begin();i!=reply_window_[clt_nonce].end();i++)
	{
		if(i->xid == xid)
		{
			i->cb_present = true;
			i->sz = sz;
			i->buf = (char*)malloc(sz);
			memcpy(i->buf,b,sz);
		}
	}
}
